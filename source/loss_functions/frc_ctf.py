...
class FourierRingCorrelationCTF(Metric):
    """
    Metric for computing the Fourier Ring Correlation (FRC) between two images with CTF weights.
    ...
    def __init__(self, size: int, **kwargs):
        """
        Initialize the FourierRingCorrelationCTF metric.
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
        """
        ...
        Args:
            ...
            weights (torch.Tensor): Weights for the images.
        ...
        weights = weights.flatten(start_dim=-2).flatten(start_dim=0, end_dim=2)[:, self.mask]

        norm_preds = (f_preds.real ** 2 + f_preds.imag ** 2) * weights
        norm_target = (f_target.real ** 2 + f_target.imag ** 2) * weights
        norm_both = (f_preds.real * f_target.real + f_preds.imag * f_target.imag) * weights
        ...
        f_weights = torch.zeros_like(num)
        for i in range(n):
            ...
            f_weights[i].index_add_(0, self.rings, weights[i])

        frc = (torch.nan_to_num(num / torch.sqrt(den1 * den2), 0) * f_weights).sum(dim=0)
        f_weights_sum = f_weights.sum(dim=0)
        self.frc += frc
        self.total += f_weights_sum
        return frc / f_weights_sum
